#!/usr/bin/env python3
"""
postgres_plan_tree_diff_cost.py
================================

Compare the query-plan trees of two folders of **PostgreSQL EXPLAIN (ANALYZE,
FORMAT JSON)** plans and report, for every matching query, a plan-tree
difference score rho.

    rho = 1 - 2*M / (|T1| + |T2|)

where |Ti| is the number of nodes in plan i's operator tree and M is the
matched mass.  rho = 0 => the plans are identical; rho -> 1 => completely
different.

This is the Postgres-explicit counterpart of duckdb_plan_tree_diff_cost.py:
same intent (run the ground-truth query and the plangen-generated query, then
score how closely their real execution plans agree), but both sides are run
and captured on **Postgres**, not DuckDB, so nothing about a columnar engine's
own optimizer shortcuts (block-skipping, dynamic filter pushdown, etc.) can
confound the comparison. The Picasso tree-matching engine below (TNode,
TreeUtil, Matcher, compare_trees, ascii_tree, ...) is IDENTICAL to the DuckDB
script -- only the JSON parser (Postgres's own EXPLAIN ANALYZE shape: "Node
Type", "Plans", "Relation Name", "Actual Rows" * "Actual Loops", ...) and the
default cost metric differ.

COST WEIGHTING (default on)
    Plain Picasso counts each matched node as exactly 1, so two structurally
    identical plans always score rho = 0 no matter how much data flowed
    through them.  Here, a matched PAIR instead contributes (1 - cost_diff) to
    M, where cost_diff in [0,1] compares the ROWS FED INTO the two nodes --
    Postgres's plain EXPLAIN ANALYZE JSON carries no output-bytes field the
    way DuckDB's 'result_set_size' does (real byte counts need BUFFERS +
    block-size math, i.e. I/O bytes, not row bytes), so the default metric
    here is rows: a node's own output size is Actual Rows * Actual Loops, and
    the bytes (rows) fed into a node are that value on its child(ren):

        cost_diff(a, b) = |rows_in(a) - rows_in(b)| / max(rows_in(a), rows_in(b))

      * unary node  -> in = the single child's row output
    ...
    This lines up with what plangen is actually built to hit exactly (row
    counts), so a plan that matches structurally but pushes a different row
    count through a shared operator still scores rho > 0.  The field is
    --cost-metric (default 'actual_rows', meaning Actual Rows * Actual Loops;
    pass a raw JSON key like 'Actual Total Time' to weight by something else
    instead -- add BUFFERS to the EXPLAIN capture for I/O-block-based metrics).
    Turn weighting off entirely with --no-cost.

The MATCHING itself is the EXACT SAME tree-difference algorithm from Picasso
...
PicassoConstants) used in duckdb_plan_tree_diff_cost.py.  Only the *parser* is
different: instead of DuckDB's EXPLAIN ANALYZE JSON, this file builds the same
style of operator tree from Postgres's `EXPLAIN (ANALYZE, FORMAT JSON)` JSON.

Folder convention (exactly like the DuckDB script):
...
Each query_<n>.json is expected to be exactly what
    psql "$DSN" -At -c "EXPLAIN (ANALYZE, FORMAT JSON) <sql>"
prints: a JSON array with one object carrying a "Plan" key, e.g.
    [{"Plan": {"Node Type": "Hash Join", ...}, "Planning Time": ..., "Execution Time": ...}]
(this is the same capture shape plan2dag/parse_postgres.py already consumes.)

Base-relation normalisation: the algorithm's leaf/join matching keys off base
relation names.  Postgres's own "Relation Name" is already the real table
name (no DuckDB-style "_0" self-join suffixing), but a trailing `_<digits>` is
still stripped by default in case a capture pipeline renamed/aliased tables
that way.  Toggle with --no-normalize.

USAGE
    python postgres_plan_tree_diff_cost.py FOLDER_A FOLDER_B
    python postgres_plan_tree_diff_cost.py FOLDER_A FOLDER_B --cost-metric "Actual Total Time"
    python postgres_plan_tree_diff_cost.py FOLDER_A FOLDER_B --no-cost       # structural only
    python postgres_plan_tree_diff_cost.py FOLDER_A FOLDER_B --csv out.csv
    python postgres_plan_tree_diff_cost.py FOLDER_A FOLDER_B --show 5        # annotated trees + costs
    python postgres_plan_tree_diff_cost.py --selftest FOLDER                 # sanity: folder vs itself -> all 0
...
class TNode:
    ...
    def __init__(self, name, attrs=None):
        ...
        # raw per-node cost metric (e.g. rows fed to parent); 0 when unavailable
        self.cost = 0.0
...
def get_join_nodes(root):
    ...
    # (the commented-out original test in TreeUtil.getJoinNodes) -- in a real
    # plan the only such operators are the join / set / union operators.
    # Scans have a single child (their relation leaf) and never qualify.
...
# in [0,1] compares the ROWS FED INTO the two nodes.  The rows fed into a node
# are its child(ren)'s own output size (Actual Rows * Actual Loops by default):
#
#     cost_diff(a, b) = |in(a) - in(b)| / max(in(a), in(b))
#
#   * unary node   -> in = the single child's output size
...
#                     leaf, whose output size is 0) -> cost_diff = 0, full match
#
...
def _input_cost_diff(n1, n2):
    """cost_diff in [0,1] from the rows fed into the pair = children's
    own output size; mean of both sides for a binary (join) node."""
...
def compare_trees(tree1, tree2, cost_weighted=True):
    ...
    identical plans that pushed very different row counts through a shared
    operator score > 0.
...
def ascii_tree(root, prefix="", last=True, show_cost=False):
    ...
    If show_cost, each node's own output size (rows fed to its parent) is shown."""
    conn = "`- " if last else "|- "
    tag = "M" if root.sim == T_IS_SIMILAR else "."
    extra = "   (rows=%g)" % root.cost if show_cost else ""
...
# ════════════════════════════════════════════════════════════════════════════
#  PostgreSQL EXPLAIN (ANALYZE, FORMAT JSON)  ->  Picasso operator tree
# ════════════════════════════════════════════════════════════════════════════
_SUFFIX_RE = re.compile(r"_\d+$")     # strips a single trailing _<digits>

# Postgres node types that scan a base relation (become: scan-operator node +
# a relation-name leaf, mirroring Picasso's Seq-Scan handling).  Kept in sync
# with plan2dag/parse_postgres.py's SCAN_TYPES.
_SCAN_TYPES = {
...
# Optional {name: canonical_name} applied to every leaf, so a plan whose tables
# were renamed by the generator (tbl_a) still matches the original (orders).
# Unmapped names pass through untouched, so it is safe to apply to both sides.
RELATION_MAP = {}


def _norm_table(name, normalize):
    name = str(name).strip().strip('"')
    # keep only the final identifier if fully-qualified (schema.table)
    if "." in name:
        name = name.split(".")[-1]
    if normalize:
        name = _SUFFIX_RE.sub("", name)      # orders_1 -> orders
    return RELATION_MAP.get(name, name)


def _op_name(node):
    """Canonical operator-node name.  Join type is folded into the name for
    join operators (Postgres already carries it directly as "Join Type", e.g.
    "Left", "Semi", "Anti"); a plain INNER join keeps the bare operator name."""
    t = str(node.get("Node Type", "")).strip()
    jt = node.get("Join Type")
    if jt and str(jt).strip().upper() not in ("", "INNER"):
        return "%s %s" % (t, str(jt).strip().upper())
    # FORMAT JSON spells every aggregate "Aggregate" and carries the strategy
    # separately, so a grouped HashAggregate and a plain COUNT(*) would
    # otherwise compare as the same operator.
    if t == "Aggregate":
        strat = str(node.get("Strategy", "") or "").strip().upper()
        if strat and strat != "PLAIN":
            return "%s %s" % (t, strat)
    return t


def _table_of(node):
    for key in ("Relation Name", "Function Name", "CTE Name",
                "Tuplestore Name"):
        v = node.get(key)
        if v:
            return v
...
# Which quantity is each node's OWN OUTPUT size.  The rows fed into a node are
# its child(ren)'s value of this.  Default 'actual_rows' means Actual Rows *
# Actual Loops (plain EXPLAIN ANALYZE always has this); pass a raw JSON key
# via --cost-metric (e.g. 'Actual Total Time', or a BUFFERS field if the
# capture used EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON)) to weight by that
# instead.
COST_METRIC = "actual_rows"


def _node_cost(node):
    if COST_METRIC == "actual_rows":
        try:
...
def _build(node, normalize):
    tn = TNode(_op_name(node))
    tn.cost = _node_cost(node)
    t = str(node.get("Node Type", "")).strip()

    children_raw = node.get("Plans", []) or []
    if t == "Bitmap Heap Scan":
        # the "Bitmap Index Scan" child is the same relation's index probe,
        # not a separate join-relevant operator -- drop it, exactly like
        # plan2dag/parse_postgres.py does for the OpNode representation.
        children_raw = [c for c in children_raw
                        if c.get("Node Type") != "Bitmap Index Scan"]

    # A scan gets its relation-name leaf FIRST (like Picasso's CreateNode),
    # then any real plan children (a scan normally has none once Bitmap Index
    # Scan is stripped, but this stays general).
...
def _find_plan_root(obj):
    """Descend past `psql ... EXPLAIN (ANALYZE, FORMAT JSON)`'s envelope
    (a JSON array holding one object with a "Plan" key) to the real plan
    root node."""
    node = obj
...
def parse_postgres_plan(path_or_obj, normalize=True):
    """Load a Postgres EXPLAIN (ANALYZE, FORMAT JSON) capture (file path or
    already-parsed object) and return its Picasso operator tree (root TNode)."""
...
def _index_folder(folder):
    ...
    (recurses one level so an inner subdir is found too)."""
    out = {}
...
def run_folders(folder_a, folder_b, normalize=True, cost_weighted=True):
    ...
    for q in common:
        try:
            t1 = parse_postgres_plan(idx_a[q], normalize)
            t2 = parse_postgres_plan(idx_b[q], normalize)
...
def _print_report(folder_a, folder_b, rows, only_a, only_b, normalize,
    ...
    print("Plan-tree difference  (Picasso GSPQO rho = 1 - 2M/(|A|+|B|))  [PostgreSQL]")
    ...
    if cost_weighted:
        ...
        if not cost_seen:
            ...
            print("      try --cost-metric \"Actual Total Time\"  (or a "
                  "BUFFERS field if captured with BUFFERS).")
    else:
...
def main():
    global COST_METRIC, RELATION_MAP
    ap = argparse.ArgumentParser(
        description="Pairwise plan-tree difference (Picasso rho) between two "
                    "folders of PostgreSQL EXPLAIN (ANALYZE, FORMAT JSON) plans.")
    ...
    ap.add_argument("--cost-metric", metavar="FIELD", default=COST_METRIC,
                    help="a node's own output size; the rows fed into a node "
                         "are its child(ren)'s value of this (default: "
                         "%(default)s = Actual Rows * Actual Loops; e.g. "
                         "'Actual Total Time', or a BUFFERS field)")
    ap.add_argument("--relation-map", metavar="FILE",
                    help="JSON object {name: canonical_name} applied to every "
                         "leaf relation, so a generator-renamed table "
                         '(e.g. {"tbl_a": "orders"}) still matches its '
                         "original; unmapped names pass through")
    ...
    if args.relation_map:
        RELATION_MAP = json.loads(open(args.relation_map).read())
    ...
    if args.show is not None:
        ...
        if q in ia and q in ib:
            t1 = parse_postgres_plan(ia[q], normalize)
            t2 = parse_postgres_plan(ib[q], normalize)
            ...
            print("\nquery_%d   rho=%.4f  M=%.3f  M_struct=%d  |A|=%d  |B|=%d"
                  % (q, r["rho"], r["matched"], r["matched_struct"],
                     r["size1"], r["size2"]))
            print("  (rows = this node's own output size = %s fed to its parent)"
                  % COST_METRIC)
